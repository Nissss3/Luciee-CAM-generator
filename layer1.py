...
if run_btn:
    ...
    # ============================================================
    # STEP 1 — Upload Bronze files to Databricks Workspace
    # ============================================================
    ...
    <div style='margin-top:1.5rem;'>
        <div style='font-size:0.7rem; font-weight:600; letter-spacing:0.14em;
                    text-transform:uppercase; color:#4A9268; margin-bottom:1rem;
                    display:flex; align-items:center; gap:0.5rem;'>
            Layer 2 Pipeline — Databricks via n8n
    ...
    STEPS = [
        {"icon": "☁️",  "label": "Upload Bronze files → Databricks /Workspace/CreditEngine/bronze/", "status": "pending"},
        {"icon": "📡",  "label": "Trigger n8n webhook",                                               "status": "pending"},
        {"icon": "🔄",  "label": "Notebook 1 — Bronze → Silver Delta tables",                         "status": "pending"},
        {"icon": "⚙️",  "label": "Notebook 2 — Silver → Gold feature engineering",                    "status": "pending"},
        {"icon": "🔐",  "label": "Notebook 3 — Gold → HE encryption → borrower_profile.json",         "status": "pending"},
        {"icon": "📥",  "label": "borrower_profile.json saved to Credit folder",                       "status": "pending"},
    ]

    def render_steps(steps):
        color_map = {"pending": "#898989", "running": "#F0A500",
                     "done": "#288147", "error": "#C0392B"}
        bg_map    = {"pending": "#F9F9F9", "running": "#FFF8EC",
                     "done": "#E8F8F0",   "error": "#FDECEA"}
        rows = ""
        for s in steps:
            c = color_map.get(s["status"], "#898989")
            b = bg_map.get(s["status"], "#F9F9F9")
            rows += f"""
            <div style='display:flex; align-items:center; gap:1rem;
                        padding:0.6rem 1rem; background:{b};
                        border-radius:8px; margin-bottom:0.35rem;
                        border:1px solid {c}22;'>
                <span style='font-size:1rem;'>{s["icon"]}</span>
                <span style='font-size:0.82rem; color:#1A3C2E; flex:1;'>{s["label"]}</span>
                <span style='font-size:0.68rem; font-weight:700; letter-spacing:0.1em;
                             text-transform:uppercase; color:{c};'>{s["status"]}</span>
            </div>"""
    ...
    # ── STEP 1: Upload Bronze files to Databricks ──────────────
    STEPS[0]["status"] = "running"
    render_steps(STEPS)

    upload_result = upload_bronze_to_databricks(company_id, BRONZE_DIR)
    ...
    if upload_result["success"]:
        ...
        # Show upload detail
        up  = upload_result["uploaded"]
        res = upload_result["results"]
        rows_html = "".join(
            f"<tr><td style='padding:0.3rem 0.6rem; color:#1A3C2E;'>{src}</td>"
            f"<td style='padding:0.3rem 0.6rem; color:#288147; font-size:0.78rem;'>"
            f"{info.get('status','')}</td>"
            f"<td style='padding:0.3rem 0.6rem; color:#898989; font-size:0.75rem;'>"
            f"{info.get('size_kb','—')} KB</td></tr>"
            for src, info in res.items()
        )
        st.markdown(f"""
        <div style='margin:0.75rem 0; background:#E8F8F0; border-radius:10px;
                    padding:0.9rem 1.2rem; border:1px solid #43AB54;'>
            <div style='font-weight:600; color:#1A3C2E; font-size:0.85rem;'>
                ☁️ {up} files uploaded to /Workspace/CreditEngine/bronze/{company_id}/
            </div>
            <table style='width:100%; margin-top:0.6rem; border-collapse:collapse;
                          font-size:0.8rem;'>
                <tr style='border-bottom:1px solid #C8E0CB;'>
                    <th style='text-align:left; padding:0.3rem 0.6rem; color:#898989;
                               font-size:0.65rem; text-transform:uppercase;'>Source</th>
                    <th style='text-align:left; padding:0.3rem 0.6rem; color:#898989;
                               font-size:0.65rem; text-transform:uppercase;'>Status</th>
                    <th style='text-align:left; padding:0.3rem 0.6rem; color:#898989;
                               font-size:0.65rem; text-transform:uppercase;'>Size</th>
                </tr>
                {rows_html}
            </table>
        </div>
        ...
        fix = upload_result.get("fix", "")
        st.markdown(f"""
        <div style='margin:0.75rem 0; background:#FDECEA; border-radius:10px;
                    padding:0.9rem 1.2rem; border:1px solid #C0392B;'>
            <div style='font-weight:600; color:#C0392B; font-size:0.85rem;'>
                ❌ Databricks Upload Failed
            </div>
            <div style='font-size:0.8rem; color:#514C50; margin-top:0.4rem;'>
                {err}
            </div>
            {"<pre style='font-size:0.75rem; background:#fff; padding:0.6rem; border-radius:6px; margin-top:0.5rem;'>" + fix + "</pre>" if fix else ""}
        </div>
        """, unsafe_allow_html=True)
        st.stop()

    # ── STEP 2: Fire n8n webhook ────────────────────────────────
    STEPS[1]["status"] = "running"
    render_steps(STEPS)

    N8N_WEBHOOK_URL = os.environ.get(
    "N8N_WEBHOOK_URL",
    "http://localhost:5678/webhook/credit-engine/trigger"
    )
    ...
    try:
        ...
        if resp.status_code in (200, 201, 202):
            n8n_ok = True
            n8n_resp = resp.json() if resp.text.strip().startswith("{") else {}
        else:
            n8n_error = f"HTTP {resp.status_code}: {resp.text[:120]}"
    except _requests.exceptions.ConnectionError:
        n8n_error = "Cannot connect to n8n — is Docker running on localhost:5678?"
    except _requests.exceptions.Timeout:
        n8n_ok    = True   # timeout is ok — n8n may have accepted but not responded
        n8n_resp  = {}
        n8n_error = None
    except Exception as e:
        n8n_error = str(e)[:150]
    ...
    if n8n_ok:
        STEPS[1]["status"] = "done"
        # Mark remaining steps as running (n8n drives them async)
        for i in range(2, len(STEPS)):
            STEPS[i]["status"] = "running"
        render_steps(STEPS)

        st.markdown(f"""
        <div style='margin-top:1rem; background:#E8F8F0; border-radius:10px;
                    padding:1rem 1.25rem; border:1px solid #43AB54;'>
            <div style='font-weight:600; color:#1A3C2E; font-size:0.9rem;'>
                ✅ Pipeline Running — Databricks is processing your data
            </div>
            <div style='font-size:0.8rem; color:#2D7A4F; margin-top:0.5rem; line-height:1.7;'>
                <b>What's happening now (via n8n):</b><br>
                🔄 Notebook 1 running — Bronze JSON → Silver Delta tables<br>
                ⏳ Notebook 2 queued — Silver → Gold feature engineering<br>
                ⏳ Notebook 3 queued — Gold → HE encryption → borrower_profile.json<br><br>
                <b>Estimated time:</b> 5–10 minutes (includes cluster warmup)<br>
                <b>Monitor progress:</b>
                <a href='http://localhost:5678' target='_blank'
                   style='color:#2D7A4F;'>n8n Dashboard</a>
                &nbsp;|&nbsp;
                Databricks → Jobs → Runs
            </div>
            <div style='margin-top:0.75rem; background:#F4FAF6; border-radius:8px;
                        padding:0.6rem 0.9rem; font-size:0.78rem; color:#3D7A59;'>
                📥 When complete, <code>borrower_profile.json</code> will be saved
                automatically to your Credit folder by n8n.<br>
                Then run: <code>python layer3_research_agent.py</code>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        render_steps(STEPS)
        st.markdown(f"""
        <div style='margin-top:1rem; background:#FFF8EC; border-radius:10px;
                    padding:1rem 1.25rem; border:1px solid #F0A500;'>
            <div style='font-weight:600; color:#1A3C2E; font-size:0.9rem;'>
                ⚠️ n8n Not Reachable — Bronze files already uploaded to Databricks
            </div>
            <div style='font-size:0.78rem; color:#514C50; margin-top:0.4rem;'>
                <b>Error:</b> {n8n_error}
            </div>
            <div style='margin-top:0.75rem; font-size:0.8rem; color:#514C50;'>
                <b>Your Bronze files ARE in Databricks</b> — just run the notebooks manually:<br>
                Databricks → Workspace → CreditEngine → open each notebook → Run All<br><br>
                <b>To start n8n:</b><br>
                <code style='background:#F4FAF6; padding:0.2rem 0.5rem; border-radius:4px;
                             font-size:0.75rem;'>
        ...
        with st.expander("🔍 Webhook Payload", expanded=False):
